...
class OolerAdvertisementData(BluetoothData):
    """Represents data from an Ooler advertisement."""
    ...
    def __init__(self) -> None:
        ...
        # formatted name like "Ooler AABB"
    ...
    def _start_update(self, data: BluetoothServiceInfo) -> None:
        ...
        if not data.name.startswith("OOLER"):
            return
        ...
        name = get_ooler_display_name(data.name, data.address)
        self.display_name = name

        self.set_title(name)
        self.set_device_name(name)
        # self.set_device_manufacturer("SNOOZ, LLC") #TODO
        # self.set_device_type("SNOOZ White Noise Machine") #TODO
        # self.set_device_hw_version(advertisement[0])


def get_ooler_display_name(local_name: str, address: str) -> str:
    # if the advertised name doesn't have any digits, then use
    # the last 4 from the mac address
    if local_name.lower() == "ooler":
        return f"Ooler {address.replace(':', '')[-4:]}"
